...
function renderGradebookTable() {
  ...
  currentStudents.forEach((student) => {
    ...
    const average =
      weeksGraded > 0 ? (totalScore / weeksGraded).toFixed(1) : "-";
...
window.exportToCsv = function () {
  ...
  const month =
    document.getElementById("monthSelect").options[
      document.getElementById("monthSelect").selectedIndex
    ].text;

  let csvContent =
    "Student Name,GitHub Username,Week 1,Week 2,Week 3,Week 4,Sprint Average\n";
  ...
  link.setAttribute(
    "download",
    `RepoReview_Grades_${section}_${month}_${year}.csv`,
  );
